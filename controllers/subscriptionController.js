...
exports.updateSubscription = async (req, res) => {
  try {
    const { subscription_status, subscription_type, expiry_date, deviceId } =
      req.body;
...
exports.handleStripeWebhook = async (req, res) => {
  ...
  try {
    ...
    switch (eventType) {
      // 3. Improve the webhook handler to better handle deviceId
      ...
      case "invoice.payment_succeeded": {
        ...
        const existingSubscription = await Subscription.getByUsername(
          customerEmail
        );
        const isRenewal =
      ...
      case "customer.subscription.updated": {
        ...
        if (!customerEmail) {
          console.warn(
            `⚠️ No email found for updated subscription of ${customerId}`
          );
          return res
            .status(400)
            .json({ message: "Email not found for subscription update" });
        }
      ...
      case "customer.subscription.deleted": {
        ...
        if (!email) {
          console.warn(
            `⚠️ No email found for deleted subscription of ${customerId}`
          );
          return res
            .status(400)
            .json({ message: "Email not found for subscription deletion" });
        }
...
async function handleSubscriptionData(
  ...
  if (isRenewal) {
    ...
    console.log(
      `✅ Subscription ${isRenewal ? "updated" : "created"} for ${email}`
    );
  }
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price:
            subscriptionType === "monthly"
              ? "price_1REoe3GKByHohIkpWa0VWsl3" // Monthly price ID
              : "price_1Rd0bVGKByHohIkp59IRLFJB", // Annual price ID
          quantity: 1,
        },
      ],
      ...
      success_url: `https://aebox-website.vercel.app/success/subscription?email=${encodeURIComponent(
        username
      )}&subscriptionType=${encodeURIComponent(
        subscriptionType
...
exports.checkSubscriptionStatus = async (req, res) => {
  ...
  try {
    ...
    if (isActive && !isExpired) {
      console.log(
        `Active subscription found for ${email}, expires: ${expiryDate}`
      );
      return res.status(200).json({
        success: true,
        active: true,
      ...
      console.log(`Subscription for ${email} is ${reason}`);
      return res.status(200).json({
        success: true,
        active: false,
        message: `Subscription is ${reason}`,
      });
    }
  } catch (error) {
